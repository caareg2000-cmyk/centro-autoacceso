...
const salasOficiales = ['Medios Digitales','Ludoteca','Diagnósticos','Lecto escritura','Sala de internet','Len 7'];
...
function renderEstadisticas(data, sala, desde, hasta) {
  ...
  actividadesOficiales.forEach(a => actividadesData[a] = data.por_actividad[a] || 0);
  ...
  const actividadMasFrecuente = Object.keys(actividadesData).reduce((a,b) => actividadesData[b] > actividadesData[a] ? b : a, Object.keys(actividadesData)[0]);
  ...
  resumenHora.innerHTML = `Promedio de registros por hora: <b>${promedioHora}</b>`;

  renderChart('graficoHora', {
    type: 'bar',
    data: {
      labels: horas,
      datasets: [{ label: 'Registros por hora', data: valoresHora }]
    },
  ...
  resumenDia.innerHTML = `Promedio de registros por día: <b>${promedioDia}</b>`;
...
function renderHeatmap(mapaCalor) {
  ...
  resumenHeatmap.innerHTML = `Mapa de concentración de asistencias. Colores más oscuros = más registros.`;
}
...
function generarColores(num){
  const colores=[];
  for(let i=0;i<num;i++) colores.push(`hsl(${(i*360)/num},70%,60%)`);
  return colores;
}

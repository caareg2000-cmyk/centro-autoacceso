...
btnCargar.addEventListener('click', async () => {
  ...
  const params = new URLSearchParams({ sala, desde, hasta });

  const resp = await fetch(`/api/estadisticas?${params.toString()}`);
...
function renderEstadisticas(data, sala, desde, hasta) {
  ...
  // ======= GRAFICO ACTIVIDADES =======
  const actividades = Object.keys(data.actividades || {});
  const valoresActividades = Object.values(data.actividades || {});
  const totalActividades = valoresActividades.reduce((a, b) => a + b, 0);
  const porcentajes = valoresActividades.map(v => totalActividades ? ((v / totalActividades) * 100).toFixed(1) : 0);

  resumenActividades.innerHTML = `
    Se registraron <b>${totalActividades}</b> actividades distintas.
    La actividad más frecuente fue <b>${actividadMasFrecuente(actividades, valoresActividades)}</b>.
  `;
  ...
  // ======= GRAFICO POR HORA =======
  const horas = Object.keys(data.porHora || {});
  const valoresHora = Object.values(data.porHora || {});
  const promedioHora = valoresHora.length ? (valoresHora.reduce((a,b)=>a+b,0) / valoresHora.length).toFixed(2) : 0;

  resumenHora.innerHTML = `
    Promedio de registros por hora: <b>${promedioHora}</b> (basado en ${valoresHora.length} horas registradas)
  `;
  ...
  // ======= GRAFICO POR DÍA =======
  const dias = Object.keys(data.porDia || {});
  const valoresDia = Object.values(data.porDia || {});
  const promedioDia = valoresDia.length ? (valoresDia.reduce((a,b)=>a+b,0) / valoresDia.length).toFixed(2) : 0;

  resumenDia.innerHTML = `
    Promedio de registros por día: <b>${promedioDia}</b> (basado en ${valoresDia.length} días registrados)
  `;
...
function renderHeatmap(mapaCalor) {
  ...
  resumenHeatmap.innerHTML = `
    Muestra la concentración de asistencias por hora y día. 
    Los colores más oscuros indican mayor número de registros.
  `;
}
...
function actividadMasFrecuente(labels, valores) {
  if (!labels.length) return 'N/A';
  const max = Math.max(...valores);
  const idx = valores.indexOf(max);
  return labels[idx];
}
...
function colorIntensidad(v) {
  // Escala de color de azul claro a rojo oscuro

...
// Fecha en formato YYYY-MM-DD, hora local CDMX
const fecha = fechaObj.toLocaleDateString('es-CA', { timeZone: 'America/Mexico_City' });

// Hora en formato HH:MM:SS, hora local CDMX
...
app.get('/api/stats', (req, res) => {
  ...
  if (from) { filter.push('fecha>=?'); params.push(from); }
  if (to) { filter.push('fecha<=?'); params.push(to); }
  if (sala) { filter.push('sala=?'); params.push(sala); }
